...
def estimate_effective_radius(radii, fluxes):
    ...
    The effective radius is the radius containing half of the maximum sampled
    flux. This matches the original TP notebook behavior and is more robust for
    background-subtracted growth curves that can decrease at large radii due to
    noise or sky oversubtraction. Linear interpolation is used between the two
    sampled radii that bracket this half-flux value.
    """
    radii = np.asarray(radii, dtype=float)

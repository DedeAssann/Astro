...
def test_aperture_growth_curve_rejects_non_finite_radii_before_measuring_fluxes():
    image = np.ones((3, 3), dtype=float)

    with pytest.raises(ValueError, match="radii must contain only finite values"):
        photometry.aperture_growth_curve(image, center=(1, 1), radii=[0, np.nan, 1])
...
def test_estimate_effective_radius_uses_max_flux_for_non_monotonic_growth_curve():
    radii = np.array([0, 1, 2, 3], dtype=float)

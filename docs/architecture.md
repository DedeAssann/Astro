...
| `scripts/make_demo_figures.py` | Post-pipeline demo-figure entry point; reads object-layout stacked FITS outputs, creates the figures directory, writes per-filter previews/histograms, prefers aligned RGB channels, and optionally writes DS9-like, named-scale, crop, and galaxy-detail-grid PNG composites. | `stacked_<filter>.png`, `histogram_<filter>.png`, `rgb_composite.png`, `rgb_composite_ds9like.png`, `rgb_composite_<limits>_<scale>.png`, `rgb_crop_<limits>_<scale>*.png`, `galaxy_detail_grid.png` |
...
| Visualization and display enhancement | Stacked FITS data or arrays. | Inspection figures, histograms, comparisons, simple RGB composites, optional enhanced RGB PNG previews, DS9-like zscale/squared composites, named display-scale composites, crop composites, and `galaxy_detail_grid.png`. | Plot helpers can save PNGs when an output path is supplied. `scripts/make_demo_figures.py` provides the object-layout CLI for discovering supported FITS files in `data/<OBJECT_NAME>/stacked/` whose stems start with `stacked_` and rendering them into `data/<OBJECT_NAME>/figures/`. RGB composition prefers `stacked/aligned_channels/stacked_<filter>_aligned.fits` when present and falls back to regular stacked files. With `--ds9like`, it writes `rgb_composite_ds9like.png` using zscale-like limits and a squared display scale. Named outputs use `--rgb-limits zscale|percentile` and `--rgb-scale linear|squared|cubed|sqrt|log|asinh|gamma`. Optional color controls use `--background-neutralization none|subtract|equalize`, `--background-percentile`, and `--color-balance none|background|median|max`; they run after channel limit scaling but before display scaling, and the CLI prints channel background estimates plus balance factors when they are active. Galaxy crop products use `--crop-center X Y` (column, row) and `--crop-size`; the crop is applied to raw channels before display transforms, Gaussian smoothing, or unsharp masking. All of these are PNG-only visualization products and never modify calibrated or stacked FITS data. |

...
| `src/astro_image_lab/enhancement.py` | Display-only post-processing helpers: zscale-like limits, percentile limits, display scales, raw-channel crops using `[x, y]` centers, Gaussian smoothing, unsharp masking, and processed RGB generation. | Normalized RGB arrays for PNG previews; no FITS products are modified. |
